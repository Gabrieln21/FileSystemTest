...
int cmd_pwd (int argcnt, char *argvec[]) {
#if (CMDPWD_ON == 1)
    char * dir_buf = malloc (DIRMAX_LEN +1); // Allocate memory for the path
    if (dir_buf == NULL) {                  // Check for allocation failure
        printf ("An error occurred: could not allocate memory\n"); 
        return -1;
    }
    ...
    if (ptr == NULL) {           //an error occurred
        ...
        free(dir_buf);            // Free the allocated memory
        return -1;
    } else {
        printf ("%s\n", ptr);
    }
